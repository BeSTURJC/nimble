...
Existen 2 paquetes diferentes e imprescindibles: 
	...
	-nimble_interfaces: Contiene los mensajes creados para almacenar una iformación concreta (para el resto se usan msgs predefinidos en ROS2) y el servicio para la generacion de trayectorias ideales (NN). 
...
Obj: publica periodicamente las medidas del paciente y los requisitos de la terapia. Se puede sustituir por publicar esos topics directamente en la terminal o una HMI.
...
Obj: Reclama el servicio de generacion de trayectorias ante cada modificacion de las medidas o requisitos. Funciones por definir para la evaluacion de las trayectoria rel vs ideal, el ajuste del nivel de asistencia en consecuencia y la trayectoria ideal de la pelvis en y,z??
		...
		-Suscribed: 
			...
			/cartesian_target (nimble_interfaces/msg/CartesianTrajectory)
				...
				en realidad el nodo recibe solo la componente x del modelo cinematico.
			...
			/step_target  (nimble_interfaces/msg/TherapyRequirements)
				altura y longitud de paso buscada (tras calculo de trayectoria ideal)
		...
		-Publishes:
			...
			/joints_target	(trajectory_msgs/msg/JointTrajectory)
   				trayectoria articular ideal obtenida por servicio (TrajGenerationService )
   				  				
   			/assistLevel  (std_msgs/Int32MultiArray) vector nivel de asistencia para cada articulacion
...
Obj: Paso de espacio articular a cartesiano del Exo, obteniendo posiciones xyz del maleolo y la pelvis, tanto de la trayectoria ideal (con long y alt de paso) como la real en cada instante
		...
		-Suscribed:
    		...
    		-Publishes:
    			/cartesian_target  (nimble_interfaces/msg/CartesianTrajectory)
				posiciones xyz para toda la trayectoria ideal tanto de la pelvis como del maleolo. 
				Este nodo calcula las componentes x, z
				
    			/cartesian_state (nimble_interfaces/msg/CartesianTrajectory)
				posiciones x, z acumuladas hasta el instante actual tanto de la pelvis como del maleolo
...
Obj: Existe un nodo implementado en simulink fuera de este paquete que gestiona la comunicacion entre ROS2 y el Exo H3 (ROS<=>CAN)
